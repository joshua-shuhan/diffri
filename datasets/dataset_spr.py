...
def get_dataloader(train=True, val=False, seed=1, num_nodes=50, batch_size=16, test_mr=0.5, gt_mr=0.0, T=100, density=0.5, noise=False, amortized=False):
    ...
    if train == True and val == False:
        dataset = spr_Dataset(train=True, val=False, seed=seed, num_nodes=num_nodes, test_mr=test_mr,
        ...
        dataset = spr_Dataset(train=False, val=False, seed=seed, num_nodes=num_nodes, test_mr=test_mr,
                              gt_mr=gt_mr, eval_length=T, density=density, noise=noise, amortized=amortized)
        loader = DataLoader(dataset, batch_size=batch_size, shuffle=1)
    elif train == True and val == True:
        dataset = spr_Dataset(train=False, val=True, seed=seed, num_nodes=num_nodes, test_mr=test_mr,
                              gt_mr=gt_mr, eval_length=T, density=density, noise=noise, amortized=amortized)
        loader = DataLoader(dataset, batch_size=batch_size, shuffle=1)
    return loader
