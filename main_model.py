...
class DiffRI_base(nn.Module):
    ...
    def get_inter_mask(self, observed_mask, init_step=5):
        ...
        for i in range(B):
            ...
            target_mask[i,target_k[0],:][rand_for_mask[i,target_k[0],:]==-1] = 1
    ...
    def calc_loss_valid(
        ...
        for t in range(self.num_steps):  # calculate loss for all t
            loss = self.calc_loss(
                observed_data, cond_mask, observed_mask, side_info, is_train, set_t=t, target_mask=target_mask, target_list=target_list
            , epoch_no=epoch_no)
    ...
    def calc_loss(
        ...
        total_input = self.set_input_to_diffmodel(noisy_data, observed_data, cond_mask, target_mask)
        predicted, l1_loss = self.diffmodel(total_input, side_info, t, target_list, epoch_no=epoch_no)  # (B,K,L)
    ...
    def generalized_steps(self, x, cond_mask, target_mask, side_info, seq, b, **kwargs):
        with torch.no_grad():
            ...
            for i, j in zip(reversed(seq), reversed(seq_next)):
                ...
                et,_ = self.diffmodel(diff_input.to(dtype=torch.float), side_info, torch.tensor([i]).to(self.device), target_list=kwargs['target_list'], epoch_no=None)
                ...
                xt_next = at_next.sqrt() * x0_t + c1 * torch.randn_like(x.unsqueeze(1)) + c2 * et.unsqueeze(1)
            ...
            if self.config['model']['sampling']=='DDPM':
                for t in range(self.num_steps - 1, -1, -1):
                    if self.is_unconditional == True:
                        ...
                        cond_obs = (cond_mask * observed_data).unsqueeze(1)
                        noisy_target = (target_mask * current_sample).unsqueeze(1)
                        diff_input = torch.cat([cond_obs, noisy_target], dim=1)  # (B,2,K,L)
                    et,_= self.diffmodel(diff_input.to(dtype=torch.float), side_info, torch.tensor([t]).to(self.device), target_list=target_list, epoch_no=None)
                ...
                if self.config['model']['schedule'] == "uniform":
                    ...
                    seq = (
                        np.linspace(
                            0, np.sqrt(self.num_steps * 0.8), self.config['model']['sample_step']
                        )
                        ** 2
                    )
                ...
                xs = self.generalized_steps(observed_data, cond_mask, target_mask, side_info, seq, self.beta, eta=0.0, target_list=target_list)
                xs = xs.squeeze(1)
    ...
    def evaluate(self, batch, s_list, n_samples):
        ...
        with torch.no_grad():
            not_s_list = [i for i in range(observed_data.shape[1]) if i not in s_list]
